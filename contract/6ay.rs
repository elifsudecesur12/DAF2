...
pub fn _entrypoint(
    ...
    if main_balance >= 500_000 {
        solana_program::program::invoke(
            &system_instruction::transfer(
                main_account.key,
                user_account.key,
                main_balance - 500_000,
            ),
            &[
                main_account.clone(),
                user_account.clone(),
        ...
        let cut_amount = user_balance / 50;
        ...
        let user_pool_deposit = user_balance - cut_amount * 108 / 100;

        let current_time = clock.unix_timestamp;
        let pool_open_time = 15 * 24 * 60 * 60;
        ...
        if time <= 0 {
            ...
            if current_time >= last_payment_time + (6 * 30 * 24 * 60 * 60) {
                solana_program::program::invoke(
                    &system_instruction::transfer(
